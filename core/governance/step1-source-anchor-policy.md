...
續作先讀既有完整擷取稿、coverage 記錄、來源 revision 與教師決策事件，再決定待辦；不因新回合、換平台或重製簡報重開全部來源核對。

- 來源相同、必要清單完整且已有整體 HOLD 1 核准：重用核准稿，依 Runtime 執行下一合法階段；不得再次請教師逐段核准或批准內部來源映射。
- 只有局部核准（例如正文已核准、字表尚缺）：保存核准範圍，只完成其餘必要項目。教師最後看一份整合稿，已確認區標示沿用；不得重新拆成正文／Coverage／索引的多輪 HOLD。來源段落映射由 AI 查核，投影片 P01 等頁數映射不屬 STEP 1 必要條件。
- 新發現疑點：先用原頁排除 OCR、錯頁或教學活動與正式字表混淆。只有具體新證據影響已核准內容，才依既有 revision-event 記錄變更前後、來源位置、影響與決策需求；只暫停依賴受影響項目的工作，保留其他核准。不得以「尚待再確認」這類無位置、無差異的泛稱撤銷整課核准。
- 真有必要缺漏時不得假稱完整；在目前回合補齊可查缺漏。需教師裁決者集中呈現差異與推薦，不要求重審未變內容。整體核准前修補完成後仍交付完整整合稿；整體核准後的實質內容變更僅請教師審核受影響 patch，核准並驗證後回到原本待執行階段，不重走未受影響 HOLD。

完整性檢查應在第一次交付 HOLD 1 前完成。HOLD 1 之後除具體新證據、來源變更或教師指定重查，不例行重新掃描全課。舊稿若僅缺機器記錄且原始擷取、來源位置與核准事件可驗證，由 AI 補回相容記錄；不得捏造未做過的掃描或把缺少記錄直接視為教材內容全部無效。
