...
任一項失敗標記 `BATCH_CONSTRUCTION_LOCK_FAIL`、`STYLE_SELECTION_REQUIRED`、`STYLE_SELECTION_HASH_MISMATCH`、`STYLE_LAYOUT_BINDING_REQUIRED`、`STYLE_LAYOUT_HASH_MISMATCH`、`STYLE_DRIFT`、`CHARACTER_REGISTRY_WRITEBACK_REQUIRED`、`CHARACTER_REGISTRY_HASH_MISMATCH`、`PAGE_DETAIL_HASH_MISMATCH`、`PAGE_SPEC_HASH_MISMATCH`、`PAGE_ORDER_DRIFT`、`PAGE_FAMILY_DRIFT`、`LAYOUT_SPEC_DRIFT`、`REPRESENTATIVE_PAGE_SELECTION_FAIL`、`REPRESENTATIVE_PAGE_SOURCE_CONFLICT`、`REPRESENTATIVE_PAGE_DETAIL_HASH_MISMATCH`、`REPRESENTATIVE_PAGE_SPEC_HASH_MISMATCH`、`REPRESENTATIVE_PAGE_FAMILY_COVERAGE_INCOMPLETE`、`PARAGRAPH_TEXT_INCOMPLETE`、`PARAGRAPH_VOCABULARY_DROPPED`、`PARAGRAPH_VOCABULARY_DETACHED`、`CLASSROOM_FONT_SIZE_UNVERIFIED`、`CLASSROOM_FONT_TOO_SMALL`、`TEXT_EMBEDDING_FAIL`、`TEXT_OBJECT_SEPARATION_FAIL`、`VOCAB_MARK_STYLE_INVALID`、`PAGE_SEQUENCE_MARKER_MISSING`、`COMIC_PANEL_PLAN_INCOMPLETE`、`UNDECLARED_PAGE` 或 `RENDER_REQUEST_UNBOUND`，整批停止，不得進入 `RENDER_READY`。
