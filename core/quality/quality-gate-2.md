...
形近字頁逐頁檢查：每頁 `character_comparison_plan.group_count` 必須為 1–2；超過兩組或沒有比較焦點時標記 `SHAPE_NEAR_GROUP_OVERFLOW`，不得用縮字或密集卡片補救。
...
14. 每頁 `layout_spec` 都回指已確認的 Style Selection `page_variant`，且 `style_variant_id`、`layout_id`、`layout_contract` 與 hash 在 PAGE_DETAIL_CONFIRMATION、Slide Script、Render Request 三層一致；版型未套用或漂移時不得進入 Render。
15. 每頁 `page_family` 都必須回指 `core/presentation/page-family-construction-contracts.md` 的 `page_family_contract_id`，並填滿該頁型的 `page_specific_plan`；契約缺失或錯配不得以通用模板補齊。
16. 形近字頁每頁只允許 1–2 組，預設 1 組；成語頁必須使用引申義／造句情境模式，`literal_image_prohibited=true`，配圖人物與動作要能回指例句。
17. 代表頁與批次檢查結果優先以 ChatGPT 原生圖像生成／影像編輯結果直接呈現於工作區，並保留可續編的原圖引用；交付前通過 `validate_native_image_review_receipt.py`。只提供 PNG 卡片、下載連結或路徑不得視為完成檢查。原生入口不可用時必須標記 `NATIVE_IMAGE_REVIEW_UNAVAILABLE`。

任一項失敗標記 `BATCH_CONSTRUCTION_LOCK_FAIL`、`STYLE_SELECTION_REQUIRED`、`STYLE_SELECTION_HASH_MISMATCH`、`STYLE_LAYOUT_BINDING_REQUIRED`、`STYLE_LAYOUT_HASH_MISMATCH`、`STYLE_DRIFT`、`CHARACTER_REGISTRY_WRITEBACK_REQUIRED`、`CHARACTER_REGISTRY_HASH_MISMATCH`、`PAGE_DETAIL_HASH_MISMATCH`、`PAGE_SPEC_HASH_MISMATCH`、`PAGE_ORDER_DRIFT`、`PAGE_FAMILY_DRIFT`、`LAYOUT_SPEC_DRIFT`、`REPRESENTATIVE_PAGE_SELECTION_FAIL`、`REPRESENTATIVE_PAGE_SOURCE_CONFLICT`、`REPRESENTATIVE_PAGE_DETAIL_HASH_MISMATCH`、`REPRESENTATIVE_PAGE_SPEC_HASH_MISMATCH`、`REPRESENTATIVE_PAGE_FAMILY_COVERAGE_INCOMPLETE`、`SHAPE_NEAR_GROUP_OVERFLOW`、`IDIOM_LITERAL_IMAGE`、`IDIOM_EXAMPLE_VISUAL_MISMATCH`、`PARAGRAPH_TEXT_INCOMPLETE`、`PARAGRAPH_VOCABULARY_DROPPED`、`PARAGRAPH_VOCABULARY_DETACHED`、`CLASSROOM_FONT_SIZE_UNVERIFIED`、`CLASSROOM_FONT_TOO_SMALL`、`TEXT_EMBEDDING_FAIL`、`TEXT_OBJECT_SEPARATION_FAIL`、`VOCAB_MARK_STYLE_INVALID`、`PAGE_SEQUENCE_MARKER_MISSING`、`COMIC_PANEL_PLAN_INCOMPLETE`、`UNDECLARED_PAGE` 或 `RENDER_REQUEST_UNBOUND`，整批停止，不得進入 `RENDER_READY`。

## Gate F｜Renderer / Regression
老師不應再手動搬字、調底線、重畫語詞範圍，也不應逐頁提醒「例句要加大」「圖要配合造句」「成語、解釋、例句不要擠在一起」。反覆出現代表 Renderer／Layout Contract 尚未完成。

代表頁必須先通過 `validate_representative_selection.py`：只能從已核准 PAGE_DETAIL 挑選，且每個實際啟用 page family 至少一頁；每頁保存 `page_detail_page_id`、PAGE_DETAIL 檔案 hash、`page_spec_sha256` 與驗證範圍。代表頁與母檔不一致時標記 `REPRESENTATIVE_PAGE_SOURCE_CONFLICT`，不得進入 `RENDER_READY`。有語詞標記時實測一次 reflow；有成語頁時至少驗證一次成語語意、例句自然度與情境圖一致性。
