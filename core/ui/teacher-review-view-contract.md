...
當簡報任務尚未有已確認的 `canvas_lock` 時，Teacher Review View 只能顯示一個畫布選擇：`4:3` 或 `16:9`。不得同時提出 3:2、9:16、平台預設或「由 AI 判斷」等選項。教師選定後，決定必須寫入 Runtime State、Slide Script、Render Request 與 Output Manifest；後續續跑不得重新詢問或自行切換。

畫布選擇未完成時，狀態為 `CANVAS_SPEC_BLOCKED`，不得建立代表頁或產生圖片。
...
若是教材未列、但已由教育部《國語辭典簡編本》核對的新增讀音，必須同時標示：

`[教材未列] [教育部《國語辭典簡編本》已核對] [AI 延伸候選，待教師確認]`

並在多音字審核表列出 `downstream_impact`，至少包含預習單、簡報腳本與學生可見頁面；教師確認前不得把它算入正式多音字列數。

每個多音字讀音都要保留可追溯證據：`source_file`、`pdf_page`、`printed_page`、`region_ref`、`exact_text`、`extraction_method`、`extraction_confidence` 與 `verification_status`。下游應引用這筆記錄，不得要求重新掃描整份 PDF。
...
審核表建議欄位：`項目｜來源狀態｜教材證據（檔名／頁碼／區塊／原文）｜字形／讀音核對｜AI 建議與理由｜受影響下游｜教師決定`。不得在同一回覆展開詩節教學、宣布新階段或預告 `STEP 2.75`。
