...
課文段落頁的語詞必須帶 `paragraph_ref`、`vocab_refs` 與 `language_placement`；預設使用 `IN_PARAGRAPH` 或 `ADJACENT_ON_SAME_PAGE`，重跑沿用既有 placement。`SEPARATE_LANGUAGE_PAGE` 只有在 Render Request 同時附具體 `reason` 與 `approval_ref` 時才合法。
