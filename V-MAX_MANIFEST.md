# V-MAX Manifest 3.8.16

## Current Canonical Files

```yaml
vmax_manifest_version: 3.8.16
...
step1_source_anchor_policy: { path: core/governance/step1-source-anchor-policy.md, current_version: 1.7 }
chinese_textbook_transcriber: { path: skills/chinese-textbook-transcriber/SKILL.md, current_version: 0.4.3 }
...
continuation_state_gate: { path: core/governance/continuation-state-gate.md, current_version: 1.4 }
...
Source Anchor Policy 1.7 是完整擷取與集中審核的 canonical：同 stage 的搜尋、逐頁／旁欄查核和存檔連續完成；必要缺口未解不得請求全文核准。Continuation State Gate 1.4 依目前階段套用前置條件，明確指定課次不被別課 active index 阻擋。教材多音字補充在 STEP 1 擷取，延伸選教與視覺鎖定依後段流程處理。
